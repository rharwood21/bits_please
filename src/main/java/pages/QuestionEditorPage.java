 ...
 * Allows the user to enter a question and its answer, and navigate to the next
 * page.
 */
public class QuestionEditorPage extends JFrame {
   ...
   public QuestionEditorPage(GameController controller) {
      ...
      String[] columnNames = { "Question", "Category", "Answer", "Multiple Choice 1", "Multiple Choice 2",
            "Multiple Choice 3", "Multiple Choice 4", "Difficulty" };
   ...
   private void deleteSelectedQuestion() {
      ...
      if (selectedRow != -1) {
         int choice = JOptionPane.showConfirmDialog(this, "Are you sure you want to delete the selected question?",
         ...
         JOptionPane.showMessageDialog(this, "Please select a question to delete.", "Delete Question",
   ...
   private Object[][] parseAndValidate() {
      ...
      for (int row = 0; row < rowCount; row++) {
         try {
            ...
            JOptionPane.showMessageDialog(this, "INVALID INPUT: All question fields must be populated.", "Error",
                  JOptionPane.ERROR_MESSAGE);
            return null;
         }
         ...
         if (!answer.equals(choice1) && !answer.equals(choice2) && !answer.equals(choice3) && !answer.equals(choice4)) {
            JOptionPane.showMessageDialog(this, "INVALID INPUT: Answer must match one of the multiple choices.",
         ...
         if (row <= currentQuestionsSize - 1) {
            ...
            if (Objects.equals(question, questionObject.getQuestionText())
                  && Objects.equals(category, questionObject.getQuestionCategory()) &&
                  Objects.equals(answer, questionObject.getQuestionAnswer())
                  && Objects.equals(choice1, questionObject.getMultipleChoiceOne()) &&
                  Objects.equals(choice2, questionObject.getMultipleChoiceTwo())
                  && Objects.equals(choice3, questionObject.getMultipleChoiceThree()) &&
                  Objects.equals(choice4, questionObject.getMultipleChoiceFour())
                  && Objects.equals(difficulty, questionObject.getQuestionDifficulty())) {
               isAltered = false;
            }
         }

         // Store the parsed values in the array
         parsedValues[row] = new Object[] { question, category, answer, choice1, choice2, choice3, choice4, difficulty,
               isAltered };
      }
   ...
   private int calculatePreferredColumnWidth(TableColumn column) {
      ...
      for (int rowIndex = 0; rowIndex < questionTable.getRowCount(); rowIndex++) {
         ...
         Component cellComponent = cellRenderer.getTableCellRendererComponent(
               questionTable, questionTable.getValueAt(rowIndex, column.getModelIndex()), false, false, rowIndex,
